...
bot.on('callback_query:data', async ctx => {
  ...
  // <Agent> watchdog: write response for <agent>-ticker.py to read (avoids getUpdates competition)
  if (data === '<agent>_watchdog_restart' || data === '<agent>_watchdog_cancel') {
    const action = data === '<agent>_watchdog_restart' ? 'restart' : 'cancel'
    try { writeFileSync('/tmp/<agent>_watchdog_response', action, 'utf8') } catch {}
    const label = action === 'restart' ? 'Reiniciando...' : 'Cancelado.'
        ...
        onStart: info => {
          ...
          void bot.api.setMyCommands(
            [
              ...
              { command: 'context', description: 'Ver contexto de sesión actual' },
              { command: 'reset', description: 'Reiniciar sesión de <Agent>' },
              { command: 'compact', description: 'Compactar contexto de sesión' },
            ],
